...
export default function DashboardLayout({
  ...
  useEffect(() => {
    // Supabase removed — keep notifications and role/department state neutral.
    if (!user) return;
    setNotifications([]);
    setStaffRole(null);
                    ...
                    <p className="text-sm font-medium text-slate-800 flex items-center justify-end gap-2">
                      ...
                      {user?.is_department_admin && (
                        <span
                          title={
                            user?.department_admin_for
                              ? `Department: ${user.department_admin_for}`
                              : "Department Admin"
                          }
        ...
        >
          ...
          >
            <ul className="space-y-2">
              {/* Role-based sidebar: compute menu based on `user.role`.
              ...
              {useMemo(() => {
                ...
                // Prefer page-provided `sidebarItems` when available so pages
                // (like admin dashboard) can control the menu. Fall back to
                // the canonical role-based menu otherwise.
                let menuToRender: {
                  label: string;
                  path: string;
                  icon: ReactNode;
                }[] = (sidebarItems && sidebarItems.length > 0) ? sidebarItems : roleMenu;
                ...
                if (user?.department === 'IQAC' && user?.role === 'hod') {
                ...
                if (
                  user &&
                  (user.role === 'hod' || user.role === 'ahod') &&
                  (String(user.department || '').toLowerCase().includes('physical') || 
                   String(user.department || '').toLowerCase() === 'pe')
                ) {
                  if (user.role === 'hod') {
                    menuToRender = [
